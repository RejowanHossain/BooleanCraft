...
(function($) {
    ...
    $(window).stellar({ responsive: true, parallaxBackgrounds: true, parallaxElements: true, horizontalScrolling: false, hideDistantElements: false, scrollProperty: 'scroll', horizontalOffset: 0, verticalOffset: 0 });
    ...
    var carousel = function() {
        $('.home-slider').owlCarousel({ loop: true, autoplay: true, margin: 0, animateOut: 'fadeOut', animateIn: 'fadeIn', nav: false, autoplayHoverPause: false, dots: false, items: 1, responsive: { 0: { items: 1 }, 600: { items: 1 }, 1000: { items: 1 } } });
        // $('.carousel-testimony').owlCarousel({ autoplay: true, autoHeight: true, center: true, loop: true, items: 1, margin: 30, stagePadding: 0, nav: false, dots: false, responsive: { 0: { items: 1 }, 600: { items: 1 }, 1000: { items: 1 } } });
    };
    carousel();
        ...
        $(window).scroll(function() {
            ...
            if (st > 350) {
                ...
                if (!navbar.hasClass('awake')) {
                     ...
                     logo.find('img.white-logo').css('display', 'none');
                     logo.prepend('<img id="theImg" class="boolean" src="images/favicon.png" />')
            ...
            if (st < 350) {
                if (navbar.hasClass('awake')) {
                    ...
                    logo.find('img.boolean').remove();
                    logo.find('img.white-logo').css('display', 'inline-block');
                }
